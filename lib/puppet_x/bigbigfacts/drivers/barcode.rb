...
class BBPFDrivers::BARCODE
  ...
  def decompressmethods
    {
      'barcode' => proc { |data, _info: {}| data } # There is no decompression method so it is disabled by returning the input. # rubocop:disable Lint/UnderscorePrefixedVariableName
    }
  end
...
#        )
  ...
  def autoload_declare
    ...
    @barcodeparts ||= Facter::Util::Bigbigpuppetfacts.loaddrivers( [ File.join(File.dirname(__FILE__), '../../../facter/util/barby-0.6.8/lib/barby/barcode/*.rb') ], opts:{:loadonly=>true,:skipclasses=>skipclasses}  )
...
#    require 'barby/barcode/upc_supplemental'
    ...
    @barcodeouts ||= Facter::Util::Bigbigpuppetfacts.loaddrivers(  [File.join(File.dirname(__FILE__), '../../../facter/util/barby-0.6.8/lib/barby/outputter/*.rb')], opts:{:loadonly=>true,:skipclasses=>skipclasses}   )
